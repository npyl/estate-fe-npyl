...
const serviceLog = (...args: any[]) => console.log("[AuthService]: ", ...args);
...
class AuthService extends DoubleStore {
    ...
    async handleAuthCallback(code: string, state: string) {
        ...
        if (res?.data?.access_token && res?.data?.expiry_date) {
            ...
            serviceLog(refreshToken, "recovered refreshToken: ");
    ...
    private async refreshAccessToken(
        ...
        try {
            ...
            serviceLog(updatedToken, "Expired! Got new: ");

            return updatedToken;
        } catch (error) {
            serviceLog("Error refreshing access token:", error);
            throw error;
        }
    }
    ...
    async getAuthForUser(userId: number): Promise<OAuth2Client | undefined> {
        ...
        if (isTokenExpired(userTokens.expiryDate)) {
            try {
                ...
                serviceLog("Token refresh failed:", error);
