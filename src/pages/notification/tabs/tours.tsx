...
const Tours = () => {
    ...
    const [sortBy, setSortBy] = useState("createdAt"); // assuming 'date' is a valid field to sort by
    const [direction, setDirection] = useState("ASC");

    const [deleteNotification, { isLoading }] = useDeleteNotificationMutation();
    const [toggleNotificationViewedStatus] =
        useToggleNotificationViewedStatusMutation();
    const [filterNotifications, { data: tours }] =
        useFilterNotificationsMutation();
    ...
    const fetchFilteredNotifications = () => {
        filterNotifications({
            filter: { types: ["TOUR"] }, // Assuming 'tour' is a valid type to filter by
            page,
            pageSize,
            sortBy,
            direction,
    ...
    useEffect(() => {
        if (tours?.content) {
            setNotifications(tours.content);
        }
    }, [tours]);
    ...
    //     await toggleNotificationViewedStatus({
    //         id: notification.id || 0,
    //         viewed: !notification.viewed, // Toggle the viewed status
    //     });

    //     setNotifications((prevNotifications) =>
    //         prevNotifications.map((notif) =>
    //             notif.id === notification.id
    //                 ? { ...notif, viewed: !notif.viewed }
    //                 : notif
    //         )
    //     );
    // };
