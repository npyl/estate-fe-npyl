...
export const CustomerSearchItem = ({ option, searchText }: SearchItemProps) => {
	...
	const firstName = useMemo(
		() =>
			option.firstName
				? parse(option.firstName, match(option.firstName, searchText))[0]
				: {
						highlight: false,
						text: "",
				  },
	...
	const mobilePhone = useMemo(
		() =>
			option.mobilePhone
				? parse(option.mobilePhone, match(option.mobilePhone, searchText))[0]
				: {
						highlight: false,
						text: "",
				  },
				...
				<Stack spacing={1} direction={"row"}>
					<Stack alignItems={"center"} direction={"row"}>
						...
						<Box
							...
							sx={{
								typography: "body2",
								fontWeight: firstName.highlight ? "bold" : "normal",
							}}
						>
							{option.firstName}
						</Box>
