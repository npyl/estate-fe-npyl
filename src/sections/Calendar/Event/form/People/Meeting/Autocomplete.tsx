...
const getPerson = ({ workspaceEmail }: IUser): TCalendarEventPerson => ({
    gwEmail: workspaceEmail,
});
...
interface AssigneeAutocompleteProps
    extends Omit<
        AutocompleteProps<IUser, true, false, false>,
        "value" | "onChange" | "options" | "renderInput"
    > {
    value: TCalendarEventPerson[]; // workspaceEmails
    onChange: (people: TCalendarEventPerson[]) => void;
...
>(({ value: _value, onChange, label, error, helperText, ...props }, ref) => {
    ...
    const handleChange = useCallback(
        (_: any, v: IUser[]) => onChange?.(v?.map(getPerson)),
        [onChange]
    );
